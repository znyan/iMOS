...
from dataset.range_transform import im_normalization, im_mean, SquarePad, Norm
...
class VOSDataset(Dataset):
    ...
    """
    def __init__(self, im_root, gt_root, max_jump, type, subset=None, num_frames=3, max_num_obj=3, finetune=False):
        ...
        for vid in vid_list:
            if subset is not None:
                if vid not in subset:
                    continue
            if type == 'ct':
                frames = sorted(os.listdir(os.path.join(self.im_root, vid)), key=lambda x: eval(x.split('.')[0]))
            elif type == 'mri':
                frames = sorted(os.listdir(os.path.join(self.im_root, vid)), key=lambda x: eval(x.split('.')[0]))
            elif type == 'endo':
                frames = sorted(os.listdir(os.path.join(self.im_root, vid)), key=lambda x: eval(x.split('.')[0]))
        ...
        self.all_im_dual_transform = transforms.Compose([
            # transforms.RandomHorizontalFlip(),
            transforms.RandomResizedCrop((384, 384), scale=(0.8,1.00), interpolation=InterpolationMode.BILINEAR)
        ])
        self.all_gt_dual_transform = transforms.Compose([
            # transforms.RandomHorizontalFlip(),
    ...
    def __getitem__(self, idx):
        ...
        while trials < 5:
            ...
            for f_idx in frames_idx:
                ...
                # this_im = self.resize_transform(this_im)
                # this_gt = self.resize_transform(this_gt)
