...
class _VOSTestDataset:
    def __init__(self, data_root, imset='val.txt'):
        self.image_dir = path.join(data_root, 'image')
        self.mask_dir = path.join(data_root, 'mask')
        self.size_dir = path.join(data_root, 'image')

        with open(path.join(data_root, imset)) as f:
