...
class XMem(nn.Module):
    def __init__(self, config, model_path=None, map_location=None):
        ...
        for name, parameter in self.key_encoder.named_parameters():
            if 'adapter' not in name:
                parameter.requires_grad = False
        for name, parameter in self.value_encoder.named_parameters():
            if 'adapter' not in name:
                parameter.requires_grad = False
        
        _need, _all = 0, 0
        for name, param in self.named_parameters():
            _param = np.prod(param.size())
            _all += _param
            if param.requires_grad:
                _need += _param
        print('params reduced by', (_all - _need) / _all)
